...
private extension SunIntervalView {
    ...
    struct Constants {
        ...
        static let dayColors: [CGColor] = {
            ...
            let middle = [UIColor](repeating: Color.day.color, count: 15)
            let suffix = [
            ...
            return (prefix + middle + suffix).map { $0.cgColor }
        }()

        static let nightColors: [CGColor] = {
            let prefix = [
                #colorLiteral(red: 0.8521052003, green: 0.217979461, blue: 0.4358110428, alpha: 1),
            ...
            let middle = [UIColor](repeating: Color.night.color, count: 15)
            let suffix = [
    ...
    func updateColors(for interval: SolarInterval) {
        ...
        let gradientLayer = layer as! CAGradientLayer // swiftlint:disable:this force_cast
